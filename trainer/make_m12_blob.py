#!/usr/bin/env python3
"""Build the M12 blob, golden vectors, and ROM self-test header.

M12: the capacity-scaling question docs/ideas-capacity-scaling-
evaluation-plan.md posed and M11.1 generated the clean baseline for --
does more model capacity (H) actually fix the standing coherence
problem (invented-word garbling), or does it just run faster/slower at
the same quality? Controlled variable: H only. Same corpus (M11.1's
shipped mix: genericized cast, lore bank OFF -- the M11.1 Part 2
winner), same seed, same training procedure (train_corpus_conditioned,
same max_epochs/patience) as make_m11_1_blob.py -- the only thing that
changes is HIDDEN (320 -> 1024) and the RSP kernel needed to run it
(rsp_ngpt.S, K-chunked, ported from the hardware-verified
docs/spikes/rsp-matvec-ktile.md spike).

H=1024, not the spike's own recommended H=768, per explicit sign-off
(Luke, 2026-07-23) -- accepted with eyes open: ~zero margin on int32
overflow (mitigated on the CPU reference path only, see core/ngpt.h),
RDRAM (rspWhh alone is 3MB of the N64's base 4MB), AND speed (the
spike's own H=1024 measurement: 5.37 ch/s, well under the M5-era
shipped bar of >=30 ch/s -- ~5 ch/s is this milestone's explicitly
accepted floor instead, not a regression nobody signed off on).

Training is cached in trainer/.m12_model.pt (git-ignored); delete it to
retrain. This one is dramatically more expensive than any prior
milestone's retrain -- 3*H*H W_hh weights scales quadratically, ~10.24x
more parameters than M11.1's H=320 (3*1024*1024 vs 3*320*320).
Run: uv run python make_m12_blob.py   (from trainer/)
"""
import random
import struct
import sys
...
CACHE = Path(__file__).resolve().parent / ".m12_model.pt"

SEED = 0
HIDDEN = 1024                # M12: the ONLY variable that changes vs M11.1
LORE_BANK_ENABLED = False    # matches M11.1's shipped winning configuration
PER_COMBO = 300
GUARD_PER_COMBO = 24
...
# M11.1's shipped H=320 numbers -- the actual controlled-comparison
# reference point this run should be judged against (same corpus, same
# seed, only H differs).
M11_1_VAL_LOSS = 0.1015
M11_1_AGREEMENT = 0.9508
...
def get_model(train_pairs, val_pairs, vocab) -> CharGRU:
    if CACHE.exists():
        ...
        return model
    # M12: device="mps", not the "cpu" forced" every prior make_mN_blob.py
    # used -- Luke's own updated CLAUDE.md policy (2026-07-23, uncommitted
    # local edit at the time of writing) allows MPS once opencoder's MLX
    # server is confirmed down right before the run (checked: `opencoder
    # status` -> DOWN), rather than never. H=1024's ~10.24x more W_hh
    # parameters than H=320 makes the CPU-only rule genuinely costly here
    # (a multi-hour run) in a way it wasn't for any prior, much smaller
    # milestone model.
...
def emit_sampler_lut_header() -> str:
    ...
    return (
        "/* AUTOGENERATED by trainer/make_m12_blob.py - do not edit.\n"
...
def emit_selftest_header(pairs: list[tuple[str, str]]) -> str:
    ...
    return (
        ...
        " * boot self-test. Must stay in sync with tests/vectors/m12_goldens.bin. */\n"
        "#pragma once\n"
        "#include <stdint.h>\n\n"
...
def main() -> None:
    ...
    print(f"corpus: {len(selena_pairs)} selena + {len(guard_pairs)} guard + "
         ...
         f"{len(full_text)/1e6:.2f} MB) -- lore_bank_enabled={LORE_BANK_ENABLED}, H={HIDDEN}")
    ...
    print(f"trained: H={HIDDEN}, val loss {model.final_loss:.4f} "
         f"(M11.1's own H=320 baseline was {M11_1_VAL_LOSS:.4f})")
    ...
    print(f"int-vs-float top-1 agreement (held-out combos, completion-only): {agree:.4f} "
         f"(M11.1's own H=320 baseline was {M11_1_AGREEMENT:.4f})")
    ...
    print(f"\ninvented-word total across {len(golden_pairs)} goldens: {total_invented} "
         f"(M11.1's H=320 26-golden probe was 69 with the lore bank off -- not the same "
         f"golden set/count, context only, not a direct bar)")
    ...
    targets = {
        ...
        REPO / "tests" / "vectors" / "m12_gru.bin": blob,
        REPO / "tests" / "vectors" / "m12_goldens.bin":
            goldens_bytes(golden_pairs),
        REPO / "tests" / "vectors" / "m12_trace.bin":
