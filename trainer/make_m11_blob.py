...
SEED = 0
HIDDEN = 320
PER_COMBO = 300              # Selena's corpus density, unchanged from M7-M10
...
def curated_golden_combos():
    ...
    one) round-trip through the real quantized engine. Unchanged from
    M10 -- gossip goldens are sampled separately below since they need a
    gossip-tagged EV:, not a curated (trust, mood, context) triple."""
...
def generalization_check(q, vocab, seed: int = SAMPLE_SEED) -> list[dict]:
    """Same falsification test as M9/M10 (unaffected in mechanism by
    M11's gossip addition -- it probes held-out (occupation, descriptor)
    pairs, orthogonal to the gossip EV: axis)."""
...
def emit_sampler_lut_header() -> str:
    ...
    return (
        "/* AUTOGENERATED by trainer/make_m11_blob.py - do not edit.\n"
...
def emit_selftest_header(pairs: list[tuple[str, str]]) -> str:
    ...
    return (
        ...
        " * boot self-test. Must stay in sync with tests/vectors/m11_goldens.bin. */\n"
        "#pragma once\n"
        "#include <stdint.h>\n\n"
...
def main() -> None:
    ...
    for prompt in gossip_golden_prompts(cast_pairs):
        got = generate_sampled(q, vocab, prompt, seed=SAMPLE_SEED,
                               inv_t_q8=INV_T_Q8, top_k=TOP_K)
        print(f"  {prompt}{got}")
        if not (1 <= len(got) <= MAX_GOLDEN_LEN):
    ...
    if div_table["identity"] < div_table["mood"] * 0.8:
        print(f"WARNING: Selena's identity divergence ({div_table['identity']:.4f}) is "
             ...
             f"catastrophic interference from M11's additions.")
    ...
    targets = {
        ...
        REPO / "tests" / "vectors" / "m11_gru.bin": blob,
        REPO / "tests" / "vectors" / "m11_goldens.bin":
            goldens_bytes(golden_pairs),
        REPO / "tests" / "vectors" / "m11_trace.bin":
