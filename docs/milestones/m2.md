...
- Weights: **int8**, symmetric, per-tensor power-of-two scale (stored as
  a right-shift amount) — shifts, not multiplies, on the N64.
...
- σ and tanh: **256-entry int16 LUTs emitted by the trainer**, input
  clamped to [−8, 8) in Q4 (index = top 8 bits after clamp), output Q14.
  No interpolation — bit-exactness beats smoothness at these sizes.
